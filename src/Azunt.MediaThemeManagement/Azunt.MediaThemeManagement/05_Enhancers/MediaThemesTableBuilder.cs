...
{
    public class MediaThemesTableBuilder
    {
        private readonly string _masterConnectionString;
        private readonly ILogger<MediaThemesTableBuilder> _logger;

        public MediaThemesTableBuilder(string masterConnectionString, ILogger<MediaThemesTableBuilder> logger)
        {
        ...
        public void BuildTenantDatabases()
        {
            ...
            foreach (var connStr in tenantConnectionStrings)
            {
                try
                {
                    EnsureMediaThemesTable(connStr);
                    _logger.LogInformation($"MediaThemes table processed (tenant DB): {connStr}");
                }
                catch (Exception ex)
                {
        ...
        public void BuildMasterDatabase()
        {
            try
            {
                EnsureMediaThemesTable(_masterConnectionString);
                _logger.LogInformation("MediaThemes table processed (master DB)");
            }
            catch (Exception ex)
            {
        ...
        private void EnsureMediaThemesTable(string connectionString)
        {
            using (var connection = new SqlConnection(connectionString))
            {
                connection.Open();

                // Check if 'MediaThemes' table exists
                var cmdCheck = new SqlCommand(@"
                    SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES 
                    WHERE TABLE_NAME = 'MediaThemes'", connection);

                int tableCount = (int)cmdCheck.ExecuteScalar();

                if (tableCount == 0)
                {
                    // Create 'MediaThemes' table if it doesn't exist
                    ...
                    _logger.LogInformation("MediaThemes table created.");
                }
                ...
                foreach (var (columnName, columnDefinition) in expectedColumns)
                {
                    var cmdColCheck = new SqlCommand(@"
                        ...
                        WHERE TABLE_NAME = 'MediaThemes' AND COLUMN_NAME = @ColumnName", connection);
                    ...
                    if (columnExists == 0)
                    {
                        var cmdAlter = new SqlCommand($@"
                            ALTER TABLE [dbo].[MediaThemes] 
                            ADD [{columnName}] {columnDefinition}", connection);
                        cmdAlter.ExecuteNonQuery();

                        _logger.LogInformation($"Column added to MediaThemes: {columnName} ({columnDefinition})");
                    }
                }

                // Insert default rows if the table is empty
                var cmdCountRows = new SqlCommand("SELECT COUNT(*) FROM [dbo].[MediaThemes]", connection);
                int rowCount = (int)cmdCountRows.ExecuteScalar();

                if (rowCount == 0)
                {
                    var cmdInsertDefaults = new SqlCommand(@"
                        INSERT INTO [dbo].[MediaThemes] (Active, IsDeleted, Created, CreatedBy, Name, DisplayOrder)
                        VALUES
                            (1, 0, SYSDATETIMEOFFSET(), 'System', 'Initial MediaTheme 1', 1),
                            (1, 0, SYSDATETIMEOFFSET(), 'System', 'Initial MediaTheme 2', 2)", connection);

                    int inserted = cmdInsertDefaults.ExecuteNonQuery();
                    _logger.LogInformation($"MediaThemes default data inserted: {inserted} rows.");
                }
            }
        }
        ...
        public static void Run(IServiceProvider services, bool forMaster, string? optionalConnectionString = null)
        {
            try
            {
                var logger = services.GetRequiredService<ILogger<MediaThemesTableBuilder>>();
                ...
                var builder = new MediaThemesTableBuilder(connectionString, logger);
            ...
            {
                var fallbackLogger = services.GetService<ILogger<MediaThemesTableBuilder>>();
                fallbackLogger?.LogError(ex, "Error while processing MediaThemes table.");
            }
        }
    }
}